...
# --- CONFIGURATION ---
MODEL_PATH = "runs/ashwagandha_detector/disease_detection_run_30_epochs/weights/best.pt"
...
def load_yolo_model(path):
    """Loads the YOLOv8 model from the specified path and caches it."""
...
def generate_pdf_report(annotated_image, predictions, care_tips_db):
    ...
    with io.BytesIO() as image_buffer:
        ...
        image_buffer.seek(0)
        pdf.image(image_buffer, x=pdf.w/2 - display_w/2, w=display_w, h=display_h)
    ...
    if not predictions:
        ...
        pdf.multi_cell(0, 10, "No specific conditions were detected by the model.", ln=1)
    else:
        grouped_preds = {p['name']: [] for p in predictions}
        for p in predictions: grouped_preds[p['name']].append(p['confidence'])
        for disease_name, confidences in grouped_preds.items():
            ...
            pdf.multi_cell(0, 6, f"Instances Found: {len(confidences)} | Average Confidence: {sum(confidences)/len(confidences):.2f}%", ln=1)
            pdf.set_font("Arial", 'B', 11)
            pdf.multi_cell(0, 6, "Suggested Action:", ln=1)
            pdf.set_font("Arial", 'I', 11)
            pdf.multi_cell(0, 6, f"  {tips_data.get('suggestion', 'N/A')}", ln=1)
            pdf.set_font("Arial", 'B', 11)
            pdf.multi_cell(0, 6, "Recommended Care Tips:", ln=1)
            pdf.set_font("Arial", '', 11)
            for tip in tips_data.get('care_tips', []):
                pdf.multi_cell(0, 6, f"  - {tip}", ln=1)
            pdf.ln(8)
    ...
    for precaution in precautions:
        pdf.multi_cell(0, 6, f"- {precaution}", ln=1)
...
def main():
    ...
    st.sidebar.info(
        "This application uses a custom-trained YOLOv8 model to detect diseases in medicinal plants. "
    ...
    st.title("🌿 Medicinal Plant Disease Diagnosis Framework")
                ...
                with st.spinner('Running diagnosis...'):
                    ...
                    with col2:
                        ...
                        if not predictions_list:
                            ...
                            pdf_report_bytes = generate_pdf_report(annotated_image, predictions_list, care_tips)
